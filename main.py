...
class Field():
    ...
    def __init__(self, U0, N, L, l, R, x, y):
        ...
        self.E = np.zeros((x+1, y+1), dtype=np.complex128)      # Distribution of the field 
                                                             # on the screen
    ...
    def make_integral_sum(self, x, y):
        ...
        for j in range(self.N):
            ...
            #fix
            nj = (qj_ - qj)*(-1*1j)/np.abs(Lj)
            a = (qj - p).real * nj.real + (qj-p).imag * nj.imag
            #fix

            #old# a = sqrt(abs(np.absolute(qj - p)**2 - t0**2))
            b = self.k/(2*self.L)
            if (a!=0):
                I, err = (quadpy.quad(lambda x: np.exp(1j * b * x**2) / 
                         (x**2 + abs(a)**2), t0, t1, epsabs=100, epsrel=100, limit=495))
                I_ = a * np.exp(1j * b * abs(a)**2) * I
                summ += I_
            else:
                summ += 0
    ...
    def snapshot(self):
        ...
        for i in range(self.y+1):
            for j in range(self.x+1):
                ...
                if (False):
                    eps = 1e-4
                    if (np.abs(self.S[i][j].real)<eps):
                        if (np.abs(self.S[i][j].imag)<eps):
                            print (self.S[i][j].real, self.S[i][j].imag, self.E[i][j], self.I[i][j])
        ...
        writer.Write()
        #U0, N, L,   l,         R,     x,   y  
f = Field(1, 3, 0.5, 0.0000007, 0.001, 50, 50)
f.make_aperture()

if (True):
  print ("hole")
  for i in range(f.A.size - 1):
    print("vert", i, np.real(f.A[i]), np.imag(f.A[i]))
